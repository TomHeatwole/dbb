 ...
 * Outcome ranges: Hwang Adjusted Positional ADP (simulator default ADP path).
 * Weekly points: Sleeper pts_half_ppr. Same rosters/slots as the Dash-rank run.
 *
 * Usage (from repo root):
 *   node scripts/run_jaml_redraft_bestball_sim_adp.mjs
 */
...
console.log('Loading players + Hwang ADP ranks…');
...
// Rank map from Hwang Adjusted Positional ADP (simulator default)
const CURRENT_YEAR = 2026;
const { loadHwangAdpRankMap, loadHwangPositionMaxRanks } = await import('../site/lib/mcp/simData.mjs');
const fullAdpMap = loadHwangAdpRankMap(CURRENT_YEAR);
const positionMaxRanks = loadHwangPositionMaxRanks(CURRENT_YEAR);
const hwangAdpRankMap = {};
for (const m of matched) {
  const info = fullAdpMap[m.pid];
  if (!info) continue;
  hwangAdpRankMap[m.pid] = info;
}
const rankedCount = Object.keys(hwangAdpRankMap).length;
const unrankedAdp = matched.filter((m) => !hwangAdpRankMap[m.pid]);
console.log(`Hwang-ADP-ranked rostered players: ${rankedCount}/${matched.length}`);
if (unrankedAdp.length) {
  console.log(`Unranked vs Hwang ADP (${unrankedAdp.length}):`);
  for (const u of unrankedAdp) console.log(`  ${u.team}: ${u.name}`);
}
...
writeFileSync(join(CACHE_DIR, 'results_adp.json'), JSON.stringify({
  iterations: ran,
  results,
  teamNames,
  unmatched,
...
console.log(`\nWrote ${join(CACHE_DIR, 'results_adp.json')}`);
