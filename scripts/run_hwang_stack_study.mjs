/**
 * QB + WR1 stacking study on the 19 Hwang archetypes.
 *
 * For each of 2021–2025 and each of the 19 competitive archetypes, jitter
 * instantiate the archetype onto that season's competitor-adjusted board
 * (same construction as the positional-value true sim). Then, from each
 * natural roster, try three paired variants that keep RBs/TEs fixed and
 * only swap QBs/WRs within a positional-rank window (≈ADP):
 *
 *   none    destack so no NFL team contributes both its QB1 and WR1
 *   single  exactly one such team-stack
 *   multi   two such team-stacks (QB1+WR1 from two different NFL teams)
 *
 * Score every roster as Hwang optimal weekly starters over 17 weeks
 * (1QB/3RB/3WR/1TE/2FLEX/1SF · 0 PPR · TE +0.5).
 *
 * Usage:
 *   npx tsx scripts/run_hwang_stack_study.mjs [seed] [builds] [outDir]
 */
import fs from 'fs';
...
const OUT_DIR = process.argv[4]
  ...
  : path.join(ROOT, 'example_data', 'hwang_stack_study');
...
function enrichBoards(seasonBoards, teamMap, adpMap) {
  const teamStars = new Map(); // year -> Map<team, {qb1, wr1}>
  for (const year of YEARS) {
    ...
    for (const pos of ['QB', 'RB', 'WR', 'TE']) {
      for (const p of board[pos] || []) {
        ...
        if (!p.team || (pos !== 'QB' && pos !== 'WR')) continue;
        const cur = stars.get(p.team) || {};
        if (pos === 'QB' && !cur.qb1) cur.qb1 = p;
        if (pos === 'WR' && !cur.wr1) cur.wr1 = p;
        stars.set(p.team, cur);
      }
    }
    for (const [team, s] of stars) {
      if (!s.qb1 || !s.wr1) stars.delete(team);
    }
    teamStars.set(year, stars);
    console.log(`  ${year}: ${stars.size} NFL teams with both a board QB1 and WR1`);
  }
  return teamStars;
}
...
function classifyStacks(players, stars) {
  ...
  for (const [team, star] of stars.entries()) {
    if (ids.has(star.qb1.sleeperId) && ids.has(star.wr1.sleeperId)) {
      stackTeams.push(team);
    }
  }
  const wrs = players.filter((p) => p.position === 'WR').sort((a, b) => a.rank - b.rank);
  const qbs = players.filter((p) => p.position === 'QB');
  const wr1 = wrs[0];
  const wr1QbStack = !!(wr1?.team && qbs.some((q) => q.team && q.team === wr1.team));
  const anySameTeam = qbs.some((q) => q.team && players.some(
    (p) => p.position === 'WR' && p.team === q.team,
  ));
  return {
...
function replacementWr(roster, slot, board, stars, rng) {
  const taken = new Set(roster.map((p) => p.sleeperId));
  taken.delete(slot.sleeperId);
  const stackedWrIds = new Set(
    [...stars.values()].filter((s) => taken.has(s.qb1.sleeperId)).map((s) => s.wr1.sleeperId),
  );
  const pool = (board.WR || []).filter((p) => (
    p.sleeperId
    && !taken.has(p.sleeperId)
    && p.team !== slot.team
    && !stackedWrIds.has(p.sleeperId)
...
function destackRoster(players, board, stars, rng) {
  ...
  for (let guard = 0; guard < 8; guard += 1) {
    ...
    for (const team of cls.stackTeams) {
      ...
      const wr = roster.find((p) => p.sleeperId === star.wr1.sleeperId);
      if (!wr) continue;
      const alt = replacementWr(roster, wr, board, stars, rng);
      if (alt && swapPlayer(roster, wr.slotIdx, alt)) {
        swapped = true;
      }
    }
    if (!swapped) break;
...
function viableStacks(roster, stars) {
  ...
  const wrSlots = roster.filter((p) => p.position === 'WR');
  const out = [];
  for (const [team, star] of stars.entries()) {
    ...
    const wrFits = wrSlots
      .filter((s) => rankFits(s.slotRank || s.rank, star.wr1.rank))
      .map((s) => ({ slot: s, cost: Math.abs((s.slotRank || s.rank) - star.wr1.rank) }))
      .sort((a, b) => a.cost - b.cost);
    if (!qbFits.length || !wrFits.length) continue;
    out.push({
      team,
      star,
      qbFits,
      wrFits,
      cost: qbFits[0].cost + wrFits[0].cost,
    });
  }
...
function applyNStacks(players, stars, nWanted) {
  ...
  for (const cand of list) {
    ...
    const wrPick = cand.wrFits.find((f) => !usedWr.has(f.slot.slotIdx)
      && (roster.find((p) => p.slotIdx === f.slot.slotIdx)?.sleeperId === cand.star.wr1.sleeperId
        || !usedIncoming.has(cand.star.wr1.sleeperId)));
    if (!qbPick || !wrPick) continue;
    const qbId = cand.star.qb1.sleeperId;
    const wrId = cand.star.wr1.sleeperId;
    // Don't assign the same incoming player to two slots.
    if (qbId === wrId) continue;
    usedQb.add(qbPick.slot.slotIdx);
    usedWr.add(wrPick.slot.slotIdx);
    usedIncoming.delete(roster.find((p) => p.slotIdx === qbPick.slot.slotIdx)?.sleeperId);
    usedIncoming.delete(roster.find((p) => p.slotIdx === wrPick.slot.slotIdx)?.sleeperId);
    usedIncoming.add(qbId);
    usedIncoming.add(wrId);
    swapPlayer(roster, qbPick.slot.slotIdx, cand.star.qb1);
    swapPlayer(roster, wrPick.slot.slotIdx, cand.star.wr1);
...
function stackNames(players, stars, teams) {
  return teams.map((team) => {
    ...
    return `${s.qb1.name}/${s.wr1.name} (${team})`;
  }).join('; ');
}
...
console.log(`QB+WR1 stack study  seed=${SEED}  builds/archetype/year=${BUILDS}`);
...
writeCsv(
  ...
  [
    ...
    'stack_class', 'stack_count', 'roster_wr1_qb_stack', 'any_qb_wr_same_team',
    'season_total', 'weekly_std', 'roster_value', 'stack_teams', 'stack_names',
    'roster_wr1_name', 'roster_wr1_team',
  ],
  buildRows,
);
...
console.log(`\n=== Overall (paired variants, RBs/TEs held fixed) ===`);
for (const v of ['none', 'single', 'multi']) {
  const s = overall[v];
  console.log(
...
console.log(
  `  roster WR1 same team as a QB: n=${wr1Split.stacked.n} median ${wr1Split.stacked.median} ≥2500 ${wr1Split.stacked.win2500}%`,
);
console.log(
  `  roster WR1 not same team:     n=${wr1Split.unstacked.n} median ${wr1Split.unstacked.median} ≥2500 ${wr1Split.unstacked.win2500}%`,
);
