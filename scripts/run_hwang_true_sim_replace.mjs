/**
 * Hwang-only replace-mode HVORP grid.
 *
 * Same clubs / scoring as v3b Hwang, but instead of adding a 27th man the
 * candidate swaps in for a similar-value player already on the built roster.
 *
 * Usage:
 *   npx tsx scripts/run_hwang_true_sim_replace.mjs [seed] [builds] [outDir]
 */
import fs from 'fs';
...
const OUT_DIR = process.argv[4]
  ...
  : path.join(ROOT, 'example_data', 'hwang_true_sim_replace');
const SEED = Number(process.argv[2]) || 1;
const BUILDS = Number(process.argv[3]) || 10;
const JITTER = 10;
const BASES = (process.env.REPLACE_BASES || 'ktc,comp').split(',').map((s) => s.trim()).filter(Boolean);
...
for (const basis of BASES) {
  console.log(`\nRunning basis="${basis}" format="hwang" replace-mode (${BUILDS} builds, seed ${SEED})…`);
  const t0 = Date.now();
  let lastPct = -10;
  ...
  const hits = results.years.reduce((s, y) => s + (y.replaceHits || 0), 0);
  const att = results.years.reduce((s, y) => s + (y.replaceAttempts || 0), 0);
  const both = results.years.reduce((s, y) => s + (y.replaceSkipBoth || 0), 0);
  const none = results.years.reduce((s, y) => s + (y.replaceSkipNoTarget || 0), 0);
  console.log(`  replace hit rate: ${att ? (hits / att * 100).toFixed(1) : 0}%  `
    + `(${hits.toLocaleString()} / ${att.toLocaleString()} ; skip both-on-roster ${both.toLocaleString()}, no-target ${none.toLocaleString()})`);
  runs.push({ basis, format: FORMAT, results });
}
...
writeCsv(
  ...
  ['value_basis', 'format', 'year', 'candidate_count', 'excluded_zero_point', 'pool_size', 'pair_count',
    'replace_attempts', 'replace_hits', 'replace_skip_both', 'replace_skip_no_target'],
  runs.flatMap(({ basis, format, results }) => results.years.map((y) => (
    [basis, format.name, y.year, y.candidateCount, y.excludedZeroPoint, y.poolSize, y.pairCount,
      y.replaceAttempts || 0, y.replaceHits || 0, y.replaceSkipBoth || 0, y.replaceSkipNoTarget || 0]
  ))),
);
...
for (const { basis, results } of runs) {
  console.log(`${basis} / hwang replace:`, results.overall.multipliers);
}
console.log('\nDone.');
