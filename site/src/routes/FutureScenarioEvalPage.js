/**
 * FutureScenarioEvalPage — the "eval" state of the Future Scenarios feature.
 *
 * Decodes the scenario param to get:
 *   - `py`  The projection season (e.g. "2024")
 *   - `c`   Roster add/drop changes
 *
 * Always loads the current live rosters (not a historical season), applies
 * the encoded roster changes, then projects a full 17-week season by mapping
 * each player's FantasyPros rank onto a historical rank equivalent.
 *
 * All display components are shared with the Scenario Builder (same output shape).
 */
...
import { decodeFutureScenario, applyScenarioChanges } from '../scenarios/scenarioEncoding';
import { validateScenarioRosters } from '../scenarios/scenarioValidation';
import { loadFpRankings } from '../scenarios/fpRankingsLoader';
...
const OG_TITLE = 'Future Scenarios — Evaluate';
const OG_DESCRIPTION = 'See projected standings for your current rosters based on FantasyPros rankings.';
...
async function loadCurrentRosterData() {
  ...
  return { teams, originalRosters, idMap, players, allWeeks };
}

// ── Invalid scenario badge (shared pattern) ────────────────────────────────────
...
function FutureScenarioEvalPage() {
  ...
  const [fpRankings, setFpRankings]                 = useState(null);
  const [historicalPositionRanks, setHistoricalPositionRanks] = useState(null);
  const [historicalWeeklyStats, setHistoricalWeeklyStats] = useState(null);
  ...
  useEffect(() => {
    const decoded = decodeFutureScenario(scenarioParam);
    if (!decoded) {
    ...
    async function load() {
      try {
        ...
        const [
          ...
          loadFpRankings(),
          fetch(`/data/stats_player_reg_${decoded.py}.csv`).catch(() => null),
          fetchMultipleWeeksStats(decoded.py, allWeeks, 0).catch(() => null),
        ]);
        ...
        // Build historical position rank arrays from the stats CSV
        let histRanks = { QB: [], RB: [], WR: [], TE: [] };
        if (statsCsvResp && statsCsvResp.ok && players) {
          const csvText = await statsCsvResp.text().catch(() => null);
          if (csvText) histRanks = buildHistoricalPositionRanks(csvText, players);
        }

        // Convert week-keyed stats { 1: {...}, 2: {...}, ... } to 0-indexed array
        const statsArray = rawHistWeeklyStats
          ? Array.from({ length: 17 }, (_, i) => rawHistWeeklyStats[i + 1] || null)
          : null;
        ...
        setFpRankings(fpRankMap);
        setHistoricalPositionRanks(histRanks);
        setHistoricalWeeklyStats(statsArray);
      } catch (e) {
  ...
  const backLink = (
    <Link
      to="/future-scenarios?state=builder"
      className="scenario-eval-back-link"
      onClick={() => {
        if (scenarioParam) {
          sessionStorage.setItem('pendingFutureBuilderScenario', scenarioParam);
        }
      }}
    >
      ← Edit Scenario
  ...
  return (
    <>
      ...
      <InfoPageWrapper
        ...
        leftHeader={backLink}
      >
        {loading && <LoadingState label="Loading projection data…" />}

        {!loading && error && (
          <div className="scenario-eval-error">
            <p>{error}</p>
            <Link to="/future-scenarios" className="scenario-eval-back-link">
              ← Back to Builder
            </Link>
          </div>
        )}
        ...
        {!loading && !error && (
          <div className="scenario-page-layout scenario-eval-layout">
            {/* Projection year badge + validity notice */}
            {projectionYear && (
              <div className="scenario-eval-topbar">
                <span className="scenario-eval-season-label">
                  Projected using {projectionYear} season stats
                </span>
                ...
                {evalResult ? (
                  ...
                  <div className="scenario-eval-placeholder">
                    ...
                    <div className="scenario-eval-placeholder-body">
                      Mapping FantasyPros ranks to {projectionYear} stats and building
            ...
            {evalResult && (
              selectedRosterId != null ? (
                ...
                <div className="scenario-eval-team-stats-placeholder">
                  Click on a team above to see their projected weekly breakdown
                </div>
              )
            )}
          </div>
...
export default FutureScenarioEvalPage;
