...
function computeTankYAxisDomain(chartData) {
...
function TankRaceCard({ currentWeekOverride = null }) {
  ...
  const [bottomTeams, setBottomTeams] = useState(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const { data, rosterIds, cumulativeTotals } = computeTankRaceSeries(
          weeksData,
          effectiveWeek,
          players,
          idMap,
        ...
        const sortedByPointsAsc = totalsArray
          .slice()
        ...
        const mappedBottom = tankTeamsRaw.map((entry) => {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          trimmedData = lastThree.map((point) => {
            ...
            const t0Key = tankRosterIds[0];
            const t1Key = tankRosterIds[1];
            return {
        ...
        setBottomTeams(orderedForDisplay);
        setChartData(trimmedData);
        setLoading(false);
      } catch (e) {
        if (!cancelled) {
          setError('Unable to load tank race data right now.');
          setBottomTeams(null);
          setChartData(null);
          setLoading(false);
        }
  ...
  const [yMin, yMax] = useMemo(() => {
    ...
    return computeTankYAxisDomain(chartData);
  }, [chartData]);
  ...
  if (loading) {
    body = (
      <div className="tank-race-status">
        <LoadingState label="Loading tank race…" />
      </div>
    );
  } else if (error) {
    body = (
  ...
  } else if (!bottomTeams || bottomTeams.length === 0) {
    body = (
      <div className="tank-race-status">
        Not enough data yet to show the tank race.
      </div>
    );
  } else {
    body = (
          ...
          <div className="tank-race-left">
            <div className="tank-race-teams">
              {bottomTeams.map((team) => (
                <div className="tank-race-team-row" key={team.rosterId}>
